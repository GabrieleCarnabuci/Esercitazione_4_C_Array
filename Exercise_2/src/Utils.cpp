...
bool ImportaVettori(const string& FileInput, double& S, size_t& n, double*& w, double*& r){
    ...
    // Salvo la grandezza dei vettori
    getline(file, inutile, ';');
    getline(file, line);
    ...
    // La terza riga è come un commento
    string header;
    getline(file, header);

    // Metto i valori letti dentro i vettori w e r
    string percentuale;
    string profitto;
    w = new double[n];
    r = new double[n];
    ...
    {
        getline(file, percentuale, ';');
        getline(file, profitto);

        istringstream convertW;
        istringstream convertR;

        convertW.str(percentuale);
        convertR.str(profitto);
